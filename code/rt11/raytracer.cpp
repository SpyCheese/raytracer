...
Color traceRay(Vector3 pos, Vector3 dir) {
	...
	if (crossObj != nullptr)
        ...
        res = Color::WHITE * 2.0 * pow(-dot(dir, world.globalLightDirection), 15.0);
	--recDepth;
	return res;
}
...
void traceLight(Vector3 const &center, double step, int cnt) {
    ...
    for (int x = -cnt; x <= cnt; ++x)
        for (int y = -cnt; y <= cnt; ++y) {
            Vector3 dir1 = dir + Vector3(
                rand() * 0.1 / RAND_MAX - 0.05,
                rand() * 0.1 / RAND_MAX - 0.05,
                rand() * 0.1 / RAND_MAX - 0.05
            );
            traceLightRay(center + dir*(-100.0) + x*v1 + y*v2, dir1, world.globalLightDiffuse);
        }
}
...
int main() {
    ...
    traceLight(Vector3(15.0, 15.0, 0.0), LIGHT_STEP, 35000);
	auto time1 = clock();
    ...
    saveImage(image, "transparency2.ppm");
