...
int main() {
    ...
    int ii = 9;
    for (double f = 0.7+0.09; f < 1.2; f += 0.01) {
        world.cameraF = f;
        vector<vector<Color>> image = render(600, 1000);
        auto time2 = clock();
        printf("%.2f: Rendered in %.3lf s\n", f, (time2 - time1) * 1. / CLOCKS_PER_SEC);
        saveImage(image, ("camera" + to_string(ii) + ".ppm").c_str());
        time1 = time2;
        ++ii;
        break;
    }
